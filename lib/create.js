...
let REMOTE_URL = 'https://github.com/xieyezi/react-template.git' // 默认为React项目模板
...
module.exports = async function create(projectName) {
  ...
  const { framework, author, description, version } = await inquirer.prompt([
    {
      type: 'list',
      name: 'framework',
      message: 'Please select one framework',
      choices: ['Vue', 'React'],
      validate(val) {
        return true
      }
  ...
  if (framework === 'Vue') {
    REMOTE_URL = 'https://github.com/xieyezi/2019-ncov-vue3-version.git'
  }
  // some tip
